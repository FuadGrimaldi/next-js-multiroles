"use client";
import { motion } from "framer-motion";

const DocsPage = () => {
  return (
    <>
      <motion.div
        className="rounded-lg border border-white bg-[#ffb800] px-8 py-11 sm:p-[55px] lg:mb-5 lg:px-[50px] lg:pb-[150px]"
        initial={{ opacity: 0, scale: 0.9 }} // Animasi saat elemen pertama kali muncul
        animate={{ opacity: 1, scale: 1 }} // Animasi ketika elemen muncul penuh
        transition={{ duration: 0.5 }} // Durasi animasi 0.5 detik
      >
        <h1 className="text-black lg:text-5xl text-2xl font-bold">
          Welcome to Startup Documentation
        </h1>

        <p className="text-black lg:text-xl text-lg mt-6">
          This document serves as a simple template to showcase a sample layout
          and format. It is solely created for demonstration purposes and is not
          intended for any official use.
        </p>
        <p className="text-black lg:text-xl text-lg mt-6">
          Please visit:{" "}
          <b>
            <a href="https://nextjstemplates.com/docs">
              nextjstemplates.com/docs
            </a>
          </b>{" "}
          to check out the real docs, setup guide and even video instructions
        </p>
      </motion.div>
    </>
  );
};

export default DocsPage;
